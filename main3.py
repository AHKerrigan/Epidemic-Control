...
env = epi_env.Epi_Env(B=0.3, graph=graph, beta_hi=0.177, beta_low=0.071, delta_low=0.20, delta_hi=0.285, action_clusters=20)
...
for i_episode in count(1):
    ...
    while num_steps < args.batch_size:
        ...
        for t in range(10000): # Don't infinite loop while learning
            ...
            running_cost = (running_cost * 0.95) + (env.last_cost * 0.05)
            reward_sum += reward
            ...
            if args.render:
                env.render()
            if done:
                running_survival = (running_survival * 0.95) + (env.last_survived * 0.05) 
                break
    ...
    if i_episode % args.log_interval == 0:
        f = open("PPO-Watts100-Cluster20.csv", "a")
        ff = open("PPO-Watts100-Cluster20hur.csv", "a")
        f.write("{}\n".format(str(reward_batch)))
        ff.write("{},{}\n".format(str(running_survival), str(running_cost)))
        f.close()
        ff.close()
